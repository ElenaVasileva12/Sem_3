...
'''
text=text.lower()
my_dict=[i for i in text.split() if i not in string.punctuation]
#print(my_dict)
my_dict_s={}
sorted_dict={}

for item in my_dict: 
    my_dict_s[item]=text.count(item)
sorted_dict = sorted(my_dict_s.items(), key=lambda x: x[1],reverse=True) # сортировка по значению
